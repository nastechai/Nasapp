...
private let backgroundRefreshIdentifier = "psyche.nasapp.background-generation.refresh"
private let backgroundProcessingIdentifier = "psyche.nasapp.background-generation.processing"
...
private final class IosBackgroundGenerationHandler {
  ...
  private func start(arguments: Any?, result: @escaping FlutterResult) {
    ...
    if args["liveActivityEnabled"] as? Bool ?? false {
      startLiveActivity(
        title: args["title"] as? String ?? "Nasapp",
  ...
  private func finish(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    let title = args["title"] as? String ?? "Nasapp"
  ...
  private func cancel(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    finishLiveActivity(
      title: liveActivityDisplayTitle.isEmpty ? "Nasapp" : liveActivityDisplayTitle,
      detail: args["detail"] as? String ?? ""
    )
    endBackgroundTask()
    resetGenerationOptions()
  ...
  private func beginBackgroundTask() {
    ...
    backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "NasappBackgroundGeneration") { [weak self] in
  ...
  private func scheduleBackgroundTasks() {
    ...
    do {
      ...
      NSLog("Nasapp background refresh schedule failed: \(error)")
    }
    ...
    do {
      ...
      NSLog("Nasapp background processing schedule failed: \(error)")
    }
  }
  ...
  private func showCompletionNotification(title: String, body: String) {
    ...
    let request = UNNotificationRequest(identifier: "nasapp.background-generation.\(Date().timeIntervalSince1970)", content: content, trigger: nil)
    UNUserNotificationCenter.current().add(request)
  }

  private func isLiveActivityActive() -> Bool {
    if #available(iOS 16.1, *) {
      return liveActivity as? Activity<NasappGenerationActivityAttributes> != nil
    }
    return false
  }
    ...
    if #available(iOS 16.1, *) {
      ...
      do {
        if #available(iOS 16.2, *) {
          liveActivity = try Activity<NasappGenerationActivityAttributes>.request(attributes: NasappGenerationActivityAttributes(title: title), content: ActivityContent(state: state, staleDate: nil), pushType: nil)
        } else {
          liveActivity = try Activity<NasappGenerationActivityAttributes>.request(attributes: NasappGenerationActivityAttributes(title: title), contentState: state, pushType: nil)
        }
        startLiveActivityRefreshTimer()
      } catch {
        NSLog("Nasapp live activity start failed: \(error)")
        liveActivity = nil
      }
    }
  }
  ...
  private func markLiveActivityFinished(title: String, detail: String) {
    if #available(iOS 16.1, *), let activity = liveActivity as? Activity<NasappGenerationActivityAttributes> {
      let finishedAt = Date()
      liveActivityDisplayTitle = title
      liveActivityDetail = detail
  ...
  private func refreshLiveActivity() {
    guard #available(iOS 16.1, *), let activity = liveActivity as? Activity<NasappGenerationActivityAttributes> else { return }
    guard !liveActivityFinished else { return }
  ...
  private func liveActivityState(
    ...
    finishedAt: Date?,
    isFinished: Bool
  ) -> NasappGenerationActivityAttributes.ContentState {
    let startedAt = liveActivityStartedAt
    let effectiveFinishedAt = finishedAt ?? Date()
    return NasappGenerationActivityAttributes.ContentState(
