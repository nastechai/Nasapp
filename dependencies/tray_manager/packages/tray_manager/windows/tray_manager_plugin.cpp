...
// Hidden helper window used as the owner of the tray
// context menu. This allows us to call TrackPopupMenu
// and SetForegroundWindow without bringing the main
// Flutter window to the front when the user opens the
// tray menu.
HWND g_tray_menu_window = nullptr;

LRESULT CALLBACK TrayMenuWndProc(HWND hwnd, UINT message, WPARAM wparam,
                                 LPARAM lparam) {
  switch (message) {
    case WM_COMMAND: {
      // Forward menu item clicks directly to Dart.
      if (channel) {
...
void TrayManagerPlugin::PopUpContextMenu(
  ...
  if (!g_tray_menu_window) {
    WNDCLASSW wc = {0};
    wc.lpfnWndProc = TrayMenuWndProc;
    wc.hInstance = GetModuleHandle(nullptr);
    wc.lpszClassName = L"TRAY_MANAGER_HELPER_WINDOW";
    RegisterClassW(&wc);

    g_tray_menu_window = CreateWindowExW(
        0, wc.lpszClassName, L"", WS_POPUP, 0, 0, 0, 0, nullptr, nullptr,
        wc.hInstance, nullptr);
  }

  HWND hWnd = g_tray_menu_window;
  ...
  // Make the (hidden) helper window the foreground window as
  // recommended by the Win32 TrackPopupMenu documentation. This
  // ensures that the menu is correctly dismissed when the user
  // clicks outside of it, without activating the main Flutter
  // window when the tray menu is opened.
  SetForegroundWindow(hWnd);

  // Use TPM_RIGHTBUTTON so both left and right clicks are handled
  // correctly by the popup, and follow the recommended pattern of
  // posting a WM_NULL message after TrackPopupMenu returns to avoid
  // focus/capture glitches that can lead to phantom menus or
  // unclickable items on some Windows versions.
